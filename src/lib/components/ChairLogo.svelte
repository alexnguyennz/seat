<script>
	...
	let mounted = false;
	onMount(async () => {
		mounted = true; // animate the logo on mount or app load, not just on route change
	});

	let drawn = false;
</script>

<svg xmlns="http://www.w3.org/2000/svg" width="80" height="111" version="1.0" viewBox="0 0 200 279"
	>{#if mounted}
		{#key pathname}
			<path
				...
				out:draw={{ duration: 1900, easing: quintInOut }}
